...
class SlidingFilter(Filter):
	...
	To each of those sliwind windows, \ref operation() is applied.
	...
	def run(self,
		...
		if radius == 0:
			return y_data
		smooth_data = copy.deepcopy(np.array(y_data))
		# Smooth the middle
		for pixel, window in utils.misc.sliding_window(y_data, radius):
			smooth_data[pixel] = self.operation(window)
		return smooth_data
	@abstractmethod
